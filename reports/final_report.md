# 最终研究报告：基于公开 Tox21 数据的可复现多终点毒性预测

日期：2026-08-14 ｜ 冻结配置：`configs/final_model.json` ｜ 结果目录：`results/final/`
阶段提交：阶段 1 `53cc814` → 阶段 2 `3e586e6` → 阶段 3（本报告所在提交即结果冻结点，之后不再依据 test 结果修改模型）
...
**数据**：(a) 审计证明该 CSV 与挑战赛原始训练库**逐分子一致**——7,823 行按 InChIKey 匹配原始 SDF，12 终点标签零冲突（`results/interim/audit/cross_version_label_agreement.csv`）；(b) 它是社区标准基准口径（TDC 同为 7,831×12），结果可与文献定性对照；(c) 文件由校验和固定，版本漂移可控。原始 SDF 含 2,952 个批次级重复组，直接建模需要自定去重规则，会把研究变成"另一个数据版本"，故仅作溯源。
...
详见 `reports/data_audit.md`。要点：类别不平衡（NR-PPAR-gamma 2.9%–SR-ARE 16.2%）；每终点 5,810–7,265 有标签（缺失=inconclusive/未测）；244 个混合物、546 个带电结构、140 个含非有机元素——真实筛选库面貌，全部保留；仅 2 组重复结构（4 行），其中二氯氰尿酸衍生物一对（TOX4994/TOX6178）在 NR-ER 上标签冲突（1 vs 0）且分处 valid/test 两侧——保留原样并写入误差分析；SR-ARE 等终点存在 valid/test 活性率漂移（15%→22–25%）。

## 4. 分子表示

**ECFP4**（Morgan 循环指纹，半径 2，2048 bit，RDKit）。对照特征 MACCS（167 bit）验证了特征选择（LogReg：0.737 vs 0.709 宏 ROC-AUC）。选择依据：该基准上文献最强经典基线特征、无外部服务依赖、完全可复现。
...
- **random 分割敏感性**（冻结后运行）：3 个随机分割 test 宏 ROC-AUC = 0.7959 / 0.8055 / 0.8205（均值 0.807，种子间 ±0.010）。scaffold 与 random 相差约 0.10 —— **分割类型的影响远大于模型间差异**，这是本研究最重要的方法学结论之一；与 MoleculeNet 论文 random 口径下强基线 ~0.82 的量级一致。
- 推理冒烟测试（`scripts/predict.py`）：磺胺苯并噻唑→AhR 1.00/ARE 0.99；睾酮→AR 0.999；阿司匹林→全部 ≈0。化学上合理。
...
- **样本**（`results/interim/error_analysis/valid_confident_errors.csv`）：最自信的假阳性是**甾体骨架分子**（TOX28569、TOX28690，AR 预测 0.999 但标注 inactive）——甾体与雄激素受体的结合是公认化学事实，这类错误更可能反映测定阈值/标注边界而非特征错误；PR-AUC 最低的 SR-ATAD5 的活性样本稀少且分散。终点难度与活性样本数无相关（valid r=-0.04），与测定自身噪声/分离度相关。
...
3. **标签噪声不可分离**：qHTS 的 inconclusive 被当作"缺失"，conflict 保留原样；终点级 AUC 的上限受测定重复性约束（NR-ER、SR-HSE 尤其）。
4. **挑战赛外部评测不可复现**：最终评测集标签从未公开，本研究与挑战赛名次不可直接比较。
5. 未使用绝对效能/剂量响应信息（Tox21 原始 qHTS 曲线可提供 AC50 等），二值化损失了信息。
...
.venv/Scripts/python -m pytest tests/ -q          # 35 个单元测试
.venv/Scripts/python scripts/audit_data.py        # 数据审计（零冲突验证等）
...
环境：Windows / Python 3.11.9 / rdkit 2026.03.5 / scikit-learn 1.9.0 / LightGBM 4.7.0 / pandas 3.0.5 / numpy 2.4.1 / torch 2.13.0+cpu（完整冻结见 `environment/requirements.txt`）。除明确列出的种子外全部流程确定性（scaffold 分割无需种子；LogReg/LGBM 确定性构建）。
